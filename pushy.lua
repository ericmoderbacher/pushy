...
function setupEmptyLine(lineNumber)
    ...
    for i=8,76,1 do lcdLines[lineNumber].message[i]=0 end
end
  ...
  midi_in.event = function(data) 
    ...
    if message.type == "cc" then
      if message.cc == 71 then 
       delta = (math.floor((message.val - 64)/math.abs(message.val - 64))* (64 - math.abs(message.val - 64)))
...
function lcdRedraw()
  send_sysex(midi_out, lcdLines[1].message)
end
function setLCDSlider(line, sliderValue)
  ...
  pos = 1
  print("string value: " .. sliderValue .. " onChar: " .. onChar .. " partials: " .. partials)
